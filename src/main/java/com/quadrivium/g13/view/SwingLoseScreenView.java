...
public class SwingLoseScreenView extends SwingGameView implements LoseScreenView{

    @Override
    protected void paintComponent(Graphics graphics){
        super.paintComponent(graphics);

        URL resource = GameJFrame.getJframe().getClass().getResource("/YouLose.png");
        BufferedImage image = null;
        try{
            image = ImageIO.read(resource);
        } catch (IOException ignored) {}

        graphics.drawImage(image, GameDimensions.getWidth()/16*8, GameDimensions.getHeight()/3*8,null);
